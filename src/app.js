...
class InvoiceProcessingApp {
  ...
  setupMiddleware() {
    ...
    this.app.use(helmet({
      contentSecurityPolicy: {
        directives: {
          defaultSrc: ["'self'"],
          styleSrc: ["'self'", "'unsafe-inline'", "https://cdn.jsdelivr.net", "https://fonts.googleapis.com", "https://cdn.mathpix.com"],
  ...
  setupRoutes() {
    ...
    this.app.get('/ping', (req, res) => {
      res.json({
        success: true,
        message: 'Invoice Processing System is running',
  ...
  setupErrorHandling() {
    ...
    this.app.use((error, req, res, next) => {
      ...
      if (process.env.NODE_ENV === 'development' && error.stack) {
        errorParams.append('stack', error.stack);
      }
      
      res.redirect(`/static/html/error.html?${errorParams.toString()}`);
    });
  ...
  async start() {
    try {
      ...
      const server = this.app.listen(this.port, '0.0.0.0', () => {
        ...
        console.log('🚀 Invoice Processing System Started');
        console.log('='.repeat(60));
